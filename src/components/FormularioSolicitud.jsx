// ============================================================
// FORMULARIO SOLICITUD v3.1 — Fintech Dark
// CAMBIOS v3.1:
//   - tipo → tipo_derecho (alineado con firebaseAdapter y flujoService)
//   - estado inicial removido del formulario (lo asigna el adapter)
//   - campos legacy eliminados: id, numero_solicitud (los genera el adapter)
// ============================================================
...
const DERECHOS_CONFIG = {
  ...
  CANCELACION:   { Icon: IconCancelacion,   color: '#EF4444', colorDim: 'rgba(239,68,68,0.12)',   glow: 'rgba(239,68,68,0.25)',   gradient: 'linear-gradient(135deg, rgba(239,68,68,0.15) 0%, rgba(239,68,68,0.03) 100%)',   artNum: '7°' },
  OPOSICION:     { Icon: IconOposicion,     color: '#8B5CF6', colorDim: 'rgba(139,92,246,0.12)',  glow: 'rgba(139,92,246,0.25)',  gradient: 'linear-gradient(135deg, rgba(139,92,246,0.15) 0%, rgba(139,92,246,0.03) 100%)',  artNum: '8°' },
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', position: 'relative' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
            ...
            <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: hovered ? color : color + '80', transition: 'color 0.2s' }}>
              Art. {artNum}
            </span>
          </div>
...
const FormularioSolicitud = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      // ── Objeto solicitud alineado con firebaseAdapter y flujoService ──
      // - tipo_derecho: nombre correcto del campo (no 'tipo')
      // - estado: lo asigna el adapter como 'PENDIENTE' (no hardcodeado aquí)
      // - numero_solicitud: lo genera el adapter (no aquí)
      // - id: lo genera Firestore (no aquí)
      const solicitud = {
        ...
        // datos del formulario (nombre_completo, rut, email, telefono, etc.)
        ...data,
        ...
        <div className="animate-fade-up" style={{ width: '100%', maxWidth: '440px', background: 'var(--bg-surface)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.08)', padding: '40px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.25)', borderRadius: '10px', padding: '14px', textAlign: 'left', marginBottom: '24px' }}>
            ...
            <p style={{ fontSize: '13px', color: '#A5B4FC', margin: 0, lineHeight: 1.5 }}>
              Revise su email y confirme su identidad con el enlace enviado. Expira en <strong>30 minutos</strong>.
            </p>
          </div>
      ...
      <div className="arcop-portal-publico" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem 1rem' }}>
        ...
        <div style={{ width: '100%', maxWidth: '640px', position: 'relative', zIndex: 1 }}>
          <div className="animate-fade-up" style={{ textAlign: 'center', marginBottom: '48px' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-secondary)', maxWidth: '380px', margin: '0 auto', lineHeight: 1.6 }}>
            ...
            {entries.map(([key, ], i) => {
              ...
              return (
                <div key={key} style={{ gridColumn: isLast ? '1 / -1' : 'auto' }}>
                  <DerechoCard keyDerecho={key} meta={meta} onSeleccionar={handleSeleccionar} index={i} />
                </div>
              );
            })}
          </div>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '24px', marginBottom: '24px' }}>
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', background: errors.acepta_terminos ? 'rgba(239,68,68,0.08)' : 'rgba(30,30,48,0.8)', border: `1px solid ${errors.acepta_terminos ? 'rgba(239,68,68,0.3)' : 'rgba(255,255,255,0.07)'}`, borderRadius: '12px', padding: '16px', cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                  Declaro que la información es verídica y acepto el tratamiento de mis datos para gestionar esta solicitud conforme a la{' '}
